...
namespace Entidades
{
    public class Producto
    {
        ...
        public static void ProductoRepetido(Producto producto)
        {
            ...
            if (Negocio.ListaVentas == producto)
            {
                int cantidadPrevia = CantidadProductoRepetido(Negocio.ListaVentas, producto);
                acumulador = cantidadPrevia + 1;

                Negocio.ListaVentas.RemoveAll(p => p.Nombre == producto.Nombre);

                producto.Cantidad = acumulador;
                producto.Precio = producto.Precio * acumulador;

                Negocio.ListaVentas.Add(producto);
            }
